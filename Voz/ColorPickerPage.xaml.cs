...
namespace Voz
{
	public partial class ColorPickerPage : PhoneApplicationPage
	{
		...
		private void lstColor_SelectionChanged ( object sender , SelectionChangedEventArgs e )
		{
			...
			{
				if ( UserData.settings.ContainKey ( AppSettings.keyAccentColor ) )
				{
					UserData.settings.Update ( AppSettings.keyAccentColor , ( ( ColorItem ) e.AddedItems[0] ).Color );
				}
				else
				{
					UserData.settings.Add ( AppSettings.keyAccentColor , ( ( ColorItem ) e.AddedItems[0] ).Color );
				}
				MessageBoxResult mesResult = MessageBox.Show ( "Restart App để dùng màu mới" );
